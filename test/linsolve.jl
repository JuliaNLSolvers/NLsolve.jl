@testset "linsolve" begin
    ...
    prob = rosenbrock()
    df = prob[1]
    res_gmres_solve = nlsolve(df, prob[2]; method=:newton, linsolve = (x, A, b)->gmres!(fill!(x, eltype(x)(0)), A, b; verbose=true))
    @test res_gmres_solve.zero ≈ res_default_solve.zero
    prob = rosenbrock()
    df = prob[1]
    res_idrs_solve = nlsolve(df, prob[2]; method=:newton, linsolve = (x, A, b)->idrs!(fill!(x, eltype(x)(0)), A, b; verbose=true))
